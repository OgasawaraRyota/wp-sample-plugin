	 ...
	 */
	 private function page_render( $args ) {
		 ...
		 if( isset( $args->image_url ) ) {
			 ...
			 $image_src = plugins_url('../images/no-image' , __FILE__ );
		 }
		 ...
		 switch( $args->how_display) {
			 case 'post_bottom':
			 	 $how_display_post[0] = 'checked';
			 	break;
			 case 'shortcode':
			 	 $how_display_shortcode[1] = 'checked';
			 	break;
			 default:
			 	break;
		 }
		 $html.='<input type="radio" name="sample-how-display" value="post_bottom" '. $how_display_post[0] .'>記事の下に表示<br>';
		 $html.='<input type="radio" name="sample-how-display" value="shortcode"' . $how_display_shortcode[1] . '>ショートコードで表示</td>';
		 $html.='</tr>';
