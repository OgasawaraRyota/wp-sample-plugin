...
class Sample_Plugin_post {
	 ...
	 */
	 private function page_render() {
		 ...
		 $html.='<tr>';
		 $html.='<th>表示方法(必須）)</th>';
		 $html.='<td><input type="radio">記事の下に表示';
		 $html.='<input type="radio">ショートコードで表示</td>';
		 $html.='</tr>';

		 $html.='<tr>';
		$html.='<th>絞り込み</th>';
		$html.='<td><input type="checkbox">カテゴリーで絞り込み';
		$html.='チェックされていない場合は、すべてに無条件で表示され、「表示するカテゴリ」項目の設定は無視されます。</td>';
		$html.='</tr>';

		 $html.='<tr>';
		 $html.='<th>表示するカテゴリ (必須)	</th>';
		 $html.='<td><input type="checkbox">カテゴリーで絞り込み';
		 $html.='選択したカテゴリーが投稿に紐づいている場合のみ画像が表示されます。</td>';
		 $html.='</tr>';
